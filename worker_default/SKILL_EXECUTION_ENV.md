...
**規格**：[`docs/design/execution-layer-v2.md`](../docs/design/execution-layer-v2.md)、[`docs/design/worker-sandbox-packages.md`](../docs/design/worker-sandbox-packages.md)、[`docs/design/harness-design.md`](../docs/design/harness-design.md) §8  
...
| 組件 | 職責 | 程式落點 |
|------|------|----------|
| **安全沙盒** | 專案根 cwd；路徑與 argv 白名單 | `modules/sandbox_runner` + kind 對應 tool policy |
| **Skill（各自管理）** | 劇本在 **`workers/<id>/skills/*.md`** | host 提供 `list_skills` / `read_skill`（過渡：`worker_runner`） |
| **角色程式（PM 可改）** | **`workers/<id>/package/`** 內 Python entrypoint | `worker_host` 載入（E-P2）；見 worker-sandbox-packages |
| **工具回饋迴圈** | Function calling → 執行 → 結果塞回對話 | host／package + Gemini（或 Scripted Agent） |
...
2. **複製種子**：自 `worker_default/{backend,scheduler}/` 複製 manifest、`SKILL.md`、`skills/`、**`package/`** 至 `workers/<id>/`（E-P1 起；過渡見 `worker_runner.copy_worker_default_into_worker_dir`）。
...
細節見 [`backend/SKILL.md`](backend/SKILL.md)、[`docs/plans/phase-e-worker-packages.md`](../docs/plans/phase-e-worker-packages.md)。
