...
- 執行層 v2（2026-06-25）：終態為 AI 產出任務順序。**Worker 種子**為 `worker_default` 下 **backend／scheduler／fixtures 三獨立 package**；可演化程式在沙盒 `workers/<id>/package/`，**PM 可改**並以專案 Git 迭代。框架僅 **worker host** 載入執行。分步實作：[`plans/phase-e-worker-packages.md`](plans/phase-e-worker-packages.md)；規格 [`design/worker-sandbox-packages.md`](design/worker-sandbox-packages.md)。
